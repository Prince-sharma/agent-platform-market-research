...
The master plan lives in `reports/agent-platform-research-plan.md`. Phase reports also live in `reports/` (`reports/agent-platform-phase1-census.md`, etc.) and feed the central interactive app (`../app/`, a React SPA deployed to GitHub Pages), whose data JSONs are regenerated from this workspace by `app/scripts/prepare_data.py`.
...
- `data/` — structured datasets. `phase1-merged.tsv` is the census (1,328 companies, 10 columns: name, sources, files, one_liner, layer, scope, vertical, status, traction, priority). `companies-with-profiles.tsv` is the census plus Phase 2 profile fields (`profile_depth`, `wiki_slug`) with verified status corrections — the app builds from this. `phase1-raw/` holds the 17 original sweep outputs. `phase2-assignments/` holds the per-agent company lists for Phase 2. `marketplaces/` holds the seven raw marketplace agent JSONs (AWS, Azure, Google, Hugging Face, NVIDIA, OpenAI, ServiceNow).
- `reports/` — the research plan, the four phase reports, and the marketplace analyses.
